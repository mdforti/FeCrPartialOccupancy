...
This is an intermetallic compound with a tetragonal structure (space group \textit{P4\textsubscript{2}/mnm}) and exhibits complex atomic arrangements due to site-specific partial occupancies.
...
\MFnew{
\paragraph{Magnetic ordering} The \textsigma-phase in the Fe-Cr is stable at high temperatures and and is observed paramagnetic at both high and room temperatures after quenching \MFnewMissRef. Therefore, we calculate elastic constants from non spin polarized calculations only, although the influence of magnetic transitions in mechanical properties will be studied later. 
}
...
We name this model PO2. % 2\_A3B\_A5B3\_AB7\_A5B3.
...
We call this other model PO1\MFmarginpar{*}{}{This is the other way around in current plots. Needs to be fixed!}.% B2\_A3B2\_A5B3\_B8\_A5B3. 
...
We extend the resulting dataset to build a a specialized Fe-Cr \textsigma-phase DFT dataset is consisting of 
...
The cell vector lengths\MFmarginpar{**}{}{This is still missing} and angles of the optimized triclinic cells are shown in \autoref{fig:SymmetryConsiderations}\MFmarginpar{***}{}{include the axis with angles to demonstrate} for all the configurations.  
Although small, the introduced structural variations might affect the mechanical properties of the \textsigma-phase. 

\begin{figure}[H]
  \subcaptionbox{\protect\label{subfig:angle_symmetry_rupture}}{\fbox{\includegraphics[height=4cm, trim=0.5cm -1cm 0.5cm 0cm, clip]{Figure_alfa_gamma_plots.pdf}}} %
...
Here we intend to use this potential in a screening tool to scan the full configurational space and select the most relevant configurations for further DFT calculations, but it does not necesarilly need be highly accurate. 
The fitting performance is shown in a table.
...
\paragraph{Elastic moduli including validation set}
 In \autoref{fig:OccupancyEffect}\MFmarginpar{*}{}{include equations, change to plot with validation points} we report the changes in the VRH \MFnewMissRef polycristalline elastic moduli with nominal composition and fractional occupation of the sublattices and its relation to formation energy.
...
%}
   \subcaptionbox{\protect\label{fig:ElasticTensorMains}}{
     \fbox{\includegraphics[height=5cm, trim = 1cm 0 1cm 1cm, clip]{Figure_C11_C13_C44_Deviations_on_equals.pdf}}
   }
   \subcaptionbox{\protect\label{fig:ElasticTensorNozero}}{
     \fbox{\includegraphics[height=5cm, trim = 1cm 0 1cm 1cm, clip]{Figure_swarm_nonzero.pdf}}
   }
   \subcaptionbox{\protect\label{fig:ElasticTensorZero}}{
     \fbox{\includegraphics[width=\linewidth, trim = 0.5cm 1cm 0.5cm 0.5cm, clip]{Figure_swarm_arezero.pdf}}
   }
...
\begin{figure}[H]
  \subcaptionbox{~}{\includegraphics[width=0.5\textwidth]{Figure_DFTDataset.pdf}}
  \subcaptionbox{~}{\includegraphics[width=0.5\textwidth]{example-image-a}}
  \caption{
