...
Atomistic studies usually assume full sublattice occupation of complete disorder, but experiments show that several Wyckoff sites are only partially occupied, creating many possible local atomic configurations.
This configurational complexity limits direct first-principles tudies and leaves open questions about how site occupancy influences the elastic properties of the \textsigma-phase.
...
We screen all possible configurations of the disordered intermetallic based on formation energy and validate the predicted relative stability and mechanical properties with DFT calculations of a representative selection of configurations. This approach enables a direct comparison between the traditional sublattice model and models that include explicit disorder at the atomic level.
...
The calculated elastic tensors differ by roughly 10\% from those predicted by symmetry-preserving models. These findings provide a clearer picture of how local chemical disorder affects the mechanical response of the \textsigma-phase and highlight the importance of including explicit configurational effects when modeling complex intermetallic compounds.
...
The \textsigma~ phase is a common intermetallic compound which precipitates usually at high temperatures in a wide variety of structural alloys \cite{hall1966sigma}, like in stainless steels \MFnewMissRef, Co based superalloys \MFnewMissRef wroght superalloys or high entropy alloys \MFnewMissRef. 
...
\MFnew{This methodologies were deviced several years ago, when a full study in the composition and configuration space was not plausible}.
...
\MFnew{
\paragraph{Magnetic ordering} The \textsigma-phase in the Fe-Cr is ferromagnetic with a Courie temperature (T$_C$) of 106K \cite{nevitt1955curie}.
Then at service temperatures avobe 870K, this phase is paramagnetic.
}
\MFnew{
Kabliman et. al. \cite{kabliman2009first} point out that the ground state of \textsigma-phase is magnetic with Feup and Crdn and small magnetic moment.
...
`Nevertheless, there are important differences when looking at the details. For instance, neglecting the magnetic entropy causes an underestimation of the Cr concentration on the and sublattices, while the and sublattices have an excess of Cr atoms in comparison with the DLM results (without the LSF), as well as the experimental data. Therefore it is clear that the magnetic entropy has a noticeable effect on the site preference of Fe and Cr atoms.'
...
We use the PO model with the lowest energy to compute the elastic constants of the sigma phase and consider that this is the best estimate.
