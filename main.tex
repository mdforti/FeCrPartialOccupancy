...
\noindent can be calculated from an optimization problem which minimizes the weighted Gibbs free energy \cite{zhang_prediction_2025}. 
...
\noindent where the indexes $i, j, k, l, m$ run over the elements Fe and Cr for each of the five sublattices. $H_{i j k l m} ^0$ are the formation enthalpies at 0K of the end-member configurations, $m_s$ are the multiplicities of the sublattices $s = 2\textit{a}, \dots 8\textit{j}$ with $m_s = 2, 4, 8, 8, 8$ respectively, and $N$ is the total number of atoms in the unit cell.
...
\begin{equation}
  \Delta H_{i,j,k,l,m}^0 = E_{i,j,k,l,m}^{\sigma} - \left( x_{i,j,k,l,m}^{Fe} E^{Fe}_{0} + x_{i,j,k,l,m}^{Cr} E^{Cr}_{0} \right)
\end{equation}

\noindent where $E_{i,j,k,l,m}^{\sigma}$  is the total energy of one unit cell of the \textsigma-phase, $E_{Fe}^{0}$ and $E_{Cr}^{0}$ are the energies per atom of Fe and Cr ground states, respectively, and $x_{i,j,k,l,m}^{Fe}$ and $x_{i,j,k,l,m}^{Cr}$ are the atomic fractions of Fe and Cr in the \textsigma-phase unit cell for the specific configuration.
The ground state of the constituent elements depend on the magnetic state to be considered. 
\MFnew{For non spin polarized calculations we consider BCC Cr and HCP Fe, while be BCC is the corresponding ground state for both Fe and Cr for spin polarized calculations.

The elastic constants for all end-members are calculated spanning the full composition range.
}
...
A first model keeps full occupancy of Fe in the sites 2\textit{a} and 8\textit{i'}, full Cr occupancy in site 4\textit{f}, but sites 8\textit{i} and 8\textit{j} will have 5 Cr and 3 Fe atoms per unit cell. We name this model PO1. 
...
A second further transfers one Cr atom from 4\textit{f} to 8\textit{i'}.
...
As show in \autoref{fig:PartialOccupancies}, there are three possible configurations with nominal composition Fe16Cr14 ($\approx$ 0.53at\% Fe). 
All of them have site 2\textit{a} filled wih Cr instead of Fe. 
...
The cell vector lengths\MFmarginpar{**}{}{This is still missing} and angles of the optimized triclinic cells are shown in \autoref{fig:SymmetryConsiderations} for all the configurations.  
Although small, the introduced structural variations might affect the mechanical properties of the \textsigma-phase. 
In \autoref{subfig:FormationEnergies} we show the results from the CEF in comparison to the calculated partial occupancy samples. 
...
\begin{figure}[H]
  ...
  \subcaptionbox{\protect\label{subfig:FormationEnergies}}{\includegraphics[height=5cm, trim=0cm 0.9cm 0cm 1cm]{Figure_ConvexHull_gibbs.pdf}}
  \caption{
    ...
    Gibbs free energies as obtained from the CEF at 0 and 873K are overimposed. 
    }
\end{figure}
...
In the other hand, the CEF at 873K shows a first crossing of the $\Delta E_f$ = 0 limit of stability, although the compositions are still shifted towards the Cr rich region. 
This might be due to the lack of electronic and vibrational contributions to he Gibbs free energies.
Those aspects might be studied in future works. 

\paragraph{fitting of GRACE}
After this first the DFT calculations set (DFT0) we fit a GRACE \MFnewMissRef potential.
Here we intend to use this potential in a screening tool to scan the full configurational space and select the most relevant configurations for further DFT calculations, but it does not necesarilly need to be highly accurate. 
The DFT extended DFT dataset is ilustrated in \autoref{fig:DFTDataset}, while the final potential performance is reported in \autoref{tab:MLIPPerformance}.


\begin{figure}[H]
  \subcaptionbox{~}{\includegraphics[height=6cm]{Figure_DFTDataset.pdf}}
  \subcaptionbox{~}{\includegraphics[height=6cm]{Figure_Workflow.pdf}}
  \caption{
...
\begin{figure}[h!]
  ...
  \subcaptionbox{\protect\label{fig:MLIPPrediction_parity}}{
		\includegraphics[height=5cm]{Figure_ParityPlot.pdf}
	}
  \subcaptionbox{\protect\label{fig:MLIPPrediction}}{
  		\includegraphics[height=5cm]{Figure_GRACE_potential_prediction.png}
	}
%
%
\end{figure}
...
\paragraph{Elastic moduli including validation set}
 In \autoref{fig:OccupancyEffect}\MFmarginpar{*}{}{include equations, change to plot with validation points} we report the changes in the polycristalline elastic moduli\MFnewMissRef with nominal composition and fractional occupation of the sublattices and its relation to formation energy.
...
%}
  ...
  \caption{\protect\label{fig:ResultsElasticConstants} 
    ...
    (\subref{fig:ElasticTensorRatios}) relative change of the elastic tensor component between the PO samples and FO sample, for non-zero valued constants in the triclinic cell (left). 
...
%}
  ...
  \caption{\protect\label{fig:ResultsElasticConstantsCEF} 
